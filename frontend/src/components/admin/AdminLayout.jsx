...
import {
  ...
  BookOpen, LogOut, Menu, X, ChevronRight, CreditCard, PlaneTakeoff, Tag
} from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { label: 'Tofly Pricing', path: '/admin/tofly-pricing', icon: Tag },
]
      ...
      <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map(({ label, path, icon: Icon, exact }) => (
          <NavLink
            ...
            title={isDesktop && !sidebarOpen ? label : undefined}
            className={({ isActive }) =>
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex">
      ...
      <motion.aside
        ...
        className="hidden lg:flex shrink-0 h-screen sticky top-0 overflow-hidden flex-col"
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed top-0 left-0 bottom-0 w-64 z-50 flex flex-col overflow-hidden"
              style={{ background: '#06060f', borderRight: '1px solid rgba(255,255,255,0.07)' }}
            >
              <SidebarContent
                sidebarOpen={true}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header
          className="h-16 border-b border-white/5 flex items-center px-4 sm:px-6 gap-4 shrink-0"
