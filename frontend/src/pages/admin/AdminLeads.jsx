...
import { Search, Filter, Trash2, ChevronDown, X, MessageCircle, Phone, Mail } from 'lucide-react'
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                {['Lead', 'Contact', 'Service', 'Budget', 'Status', 'Date', ''].map(h => (
              ...
              {isLoading ? (
                ...
                data.leads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="text-xs text-white/55">{lead.email}</div>
                      {lead.phone && <div className="text-xs text-white/35 mt-0.5">{lead.phone}</div>}
                    </td>
                    <td className="px-5 py-4 text-xs text-white/45 capitalize">
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-brand-500/15 flex items-center justify-center text-xl font-bold text-brand-300">
                      {selectedLead.name?.[0]}
                    </div>
                    <div>
                      <div className="text-lg font-bold text-white">{selectedLead.name}</div>
                      {selectedLead.company && <div className="text-sm text-white/40">{selectedLead.company}</div>}
                    </div>
                  </div>
                  ...
                  {[
                    ...
                    <div key={label} className="flex justify-between py-3 border-b border-white/4">
                      <span className="text-sm text-white/35">{label}</span>
                      <span className="text-sm text-white/75 capitalize text-right max-w-[60%]">{value}</span>
                    </div>
                  ))}
