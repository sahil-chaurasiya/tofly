...
export default function AdminTestimonials() {
  ...
  const toggleFeatured = useMutation({
    mutationFn: ({ id, isFeatured }) => adminAPI.updateTestimonial(id, { isFeatured, isActive: true }),
    onSuccess: (_, { isFeatured }) => {
      qc.invalidateQueries(['admin-testimonials'])
      toast.success(isFeatured ? 'Featured on homepage' : 'Removed from homepage')
    },
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">Testimonials</h1>
          <p className="text-white/40 text-sm">{testimonials?.length || 0} testimonials</p>
        </div>
        <button onClick={openNew} className="btn-primary text-sm">
          <Plus size={16} /> <span className="hidden sm:inline">Add Testimonial</span><span className="sm:hidden">Add</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
        {isLoading ? (
          ...
          testimonials.map(t => (
            <div key={t._id} className="glass-card p-4 sm:p-6 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="font-semibold text-white text-sm truncate">{t.name}</div>
                  <div className="text-xs text-white/40 mt-0.5 truncate">{t.designation}{t.company ? `, ${t.company}` : ''}</div>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => toggleFeatured.mutate({ id: t._id, isFeatured: !t.isFeatured })}
                    className={'p-1.5 rounded-lg transition-colors ' + (t.isFeatured ? 'text-amber-400 bg-amber-500/10 hover:bg-amber-500/20' : 'text-white/25 hover:text-amber-400 hover:bg-amber-500/10')}
                    title={t.isFeatured ? 'Remove from homepage' : 'Feature on homepage'}
                  >
                    <Star size={14} className={t.isFeatured ? 'fill-amber-400' : ''} />
                  </button>
                  <button
                    onClick={() => openEdit(t)}
          ...
          <div className="sm:col-span-2 xl:col-span-3 text-center py-20 text-white/25 text-sm">
            No testimonials yet.{' '}
