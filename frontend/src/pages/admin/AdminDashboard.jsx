...
function StatCard({ label, value, icon: Icon, color, path, loading }) {
  return (
    <Link to={path} className="block">
      <motion.div
        whileHover={{ y: -2 }}
        className="glass-card p-5 sm:p-6 cursor-pointer border hover:border-brand-500/25 transition-all duration-200"
      >
        ...
        {loading ? (
          ...
          <div className="text-2xl sm:text-3xl font-black font-display text-white mb-1">{value}</div>
        )}
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {leadsData?.stats && (
        <div className="glass-card p-4 sm:p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base sm:text-lg font-bold text-white">Lead Pipeline</h2>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {['new', 'contacted', 'qualified', 'proposal-sent', 'converted', 'lost'].map(status => {
              ...
              return (
                <div key={status} className={`px-3 sm:px-4 py-2 rounded-xl border text-xs sm:text-sm font-medium ${STATUS_COLORS[status] || 'bg-white/5 text-white/40 border-white/10'}`}>
      ...
      {leadsData?.leads?.length > 0 && (
        <div className="glass-card overflow-hidden">
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-white/5">
            <h2 className="text-base sm:text-lg font-bold text-white">Recent Leads</h2>
            <Link to="/admin/leads" className="text-sm text-brand-400 hover:text-brand-300 transition-colors flex items-center gap-1">
              ...
              <thead>
                <tr className="border-b border-white/5">
                  {['Name', 'Email', 'Service', 'Status', 'Date'].map(h => (
                    <th key={h} className="text-left text-xs font-semibold text-white/35 uppercase tracking-wider px-4 sm:px-6 py-4">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-white truncate max-w-[120px]">{lead.name}</div>
                          {lead.company && <div className="text-xs text-white/35 truncate max-w-[120px]">{lead.company}</div>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 text-sm text-white/55 max-w-[160px]">
                      <div className="truncate">{lead.email}</div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 text-sm text-white/40 capitalize whitespace-nowrap">{lead.service?.replace(/-/g, ' ')}</td>
                    <td className="px-4 sm:px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border whitespace-nowrap ${STATUS_COLORS[lead.status] || 'bg-white/5 text-white/40 border-white/10'}`}>
                        {lead.status}
                      </span>
                    </td>
                    <td className="px-4 sm:px-6 py-4 text-xs text-white/30 whitespace-nowrap">
      ...
      {/* Quick actions */}
      <div>
        <h2 className="text-base sm:text-lg font-bold text-white mb-4">Quick Actions</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            <Link key={path} to={path} className="glass-card p-4 sm:p-5 hover:border-brand-500/25 transition-all duration-200 block group">
              <Icon size={20} className="text-white/40 group-hover:text-brand-400 transition-colors mb-3" />
              <div className="text-sm font-semibold text-white mb-1 leading-tight">{label}</div>
              <div className="text-xs text-white/35 hidden sm:block">{desc}</div>
            </Link>
          ))}
        </div>
      </div>
