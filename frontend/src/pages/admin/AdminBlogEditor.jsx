...
export default function AdminBlogEditor() {
  ...
  const { data: existingBlog } = useQuery({
    ...
    queryFn: () => adminAPI.getBlogs({ limit: 100 }).then(r =>
      r.data.blogs.find(b => b._id === id)
    ),
    enabled: isEdit,
    onSuccess: (blog) => {
      if (blog) {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/admin/blogs" className="p-2 rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
            <ArrowLeft size={18} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-white">{isEdit ? 'Edit Post' : 'New Blog Post'}</h1>
            <p className="text-white/35 text-sm mt-0.5">{isEdit ? 'Update and republish' : 'Create and publish a new article'}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 space-y-5">
            <div>
              ...
              <input
                ...
                className={`input-field text-lg font-semibold ${errors.title ? 'border-red-500/50' : ''}`}
              />
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card p-5">
            ...
            {uploading && <p className="text-xs text-brand-400">Uploading...</p>}
            <div>
